...
func (e *Endpoint) removeOldFilter(owner Owner, labelsMap *identityPkg.IdentityCache,
	...
	for _, sel := range filter.FromEndpoints {
		for _, id := range getSecurityIdentities(labelsMap, &sel) {
			...
			if err := e.SecurityIdentityMap.DeleteL4(srcID, port, proto, policy.Ingress.Uint8()); err != nil {
...
func (e *Endpoint) applyNewFilter(owner Owner, labelsMap *identityPkg.IdentityCache,
	...
	for _, sel := range filter.FromEndpoints {
		for _, id := range getSecurityIdentities(labelsMap, &sel) {
			srcID := id.Uint32()
			if e.SecurityIdentityMap.L4Exists(srcID, port, proto, policy.Ingress.Uint8()) {
			...
			if err := e.SecurityIdentityMap.AllowL4(srcID, port, proto, policy.Ingress.Uint8()); err != nil {
...
// this Endpoint's BPF SecurityIdentityMap to reflect the new L3+L4 combined policy.
...
func (e *Endpoint) regenerateConsumable(owner Owner, labelsMap *identityPkg.IdentityCache,
	...
	if e.L4Policy == c.L4Policy && e.LabelsMap == labelsMap {
		...
		// SecurityIdentityMap can't be created in dry mode.
...
func (e *Endpoint) regeneratePolicy(owner Owner, opts models.ConfigurationMap) (bool, policy.SecurityIDContexts, policy.SecurityIDContexts, error) {
	...
	if e.SecurityIdentityMap == nil && !owner.DryModeEnabled() {
	...
	// Need to regenerate BPF for this endpoint if any options for it have
	// changed, // if the policy itself has changed, or the revision number for
	// its policy has increased.
